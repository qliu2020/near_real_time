...
% Initialize joint angular accelerations using for load calculation
...
while tIndex<n_sample-window_size
    ...
    if calib_status == 0
        ...
        if mod(ind_after_calib,overlap) == 1
            ...
            label = mode(ind)-1;
            k = (ind_after_calib-1)*n+1:(ind_after_calib-1+overlap)*n;
            output_label(k) = label;
        end
        
        % Calculate joitn load
        lefthip_angle_acc = rad2deg(diff(lefthip_angularVel([1,n],:)))*fs/(n-1);
        lhload = sum(lefthip_angle_acc.^2)/1e8;
                
        righthip_angle_acc = rad2deg(diff(righthip_angularVel([1,n],:)))*fs/(n-1);
        rhload = sum(righthip_angle_acc.^2)/1e8;  
    end
end
